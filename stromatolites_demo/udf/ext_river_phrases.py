...
#initalize the river_phrases table
cursor.execute("""
    DELETE FROM river_phrases;
""")
...
with open('./var/river_variables.txt') as fid:
...
#write to PSQL table
cursor.executemany("""
    INSERT INTO river_phrases(    docid,
                                  sentid,
                                  strat_phrase,
                                  strat_phrase_root,
                                  strat_flag,
...
cursor.execute("""  VACUUM ANALYZE river_phrases;""")
connection.commit()

#initalize the strat_dict table
#cursor.execute("""DELETE FROM strat_dict;""")
...
for idx1,doc in enumerate(doc_list.keys()):
    ...
    #cursor.execute("""INSERT INTO strat_dict(docid,strat_phrase) VALUES (%s, %s);""",(doc, strat_doc))
